...
- ゴミ箱を空にする（件数確認後、自分の削除済み付箋・タスクをアプリから復元不可にします）。[操作と保存先の扱い](docs/empty-trash.md)
- 目のアイコンでタイトル・内容を非表示。受信付箋は初期状態で非表示。[付箋の非表示](docs/note-visibility.md) を参照してください。
- 本文の表を直接編集（行・列追加、表貼り付け、太字）。操作は [付箋の表編集](docs/note-editor.md) を参照してください。
- 写真の貼り付け（JPEG・PNG、自動縮小、3枚まで）、既存付箋の「送る」「投稿する」。[写真・送信・投稿の手順](docs/photo-sharing.md) を参照してください。
...
写真・大きな本文の保存には **StickyNotesの添付ファイルを有効** にしてください。新しいリスト・列は不要です。大きな暗号化データはアプリが自動作成する保管用行（NoteType=BLOB）の添付として保存します。この保管用行と添付を手動削除しないでください。リストのバックアップには添付も含めてください。
## リスト別の列型
