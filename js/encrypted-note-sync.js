...
function syncEncryptedDirectNote(note, names) {
    ...
    if (names.length === 1 && /^[A-Za-z0-9]+$/.test(String(names[0]).replace(/^\s+|\s+$/g, ''))) { StickyGroupsApi.findMembers(String(names[0]).replace(/^\s+|\s+$/g, ''), function (group) { if (!group.ok || !group.users.length) { Fsn.toast('部署の所属ユーザーが見つかりません。'); return; } syncEncryptedRecipients(note, group.users); }); return; }
    ...
    StickyUsersApi.findByDisplayNames(expanded, function (lookup) {
        ...
        syncEncryptedRecipients(note, recipients, contentKey, payload);
    });
}
function syncEncryptedRecipients(note, recipients, contentKey, payload) {
    var user = Storage.get('sticky_user', {}), i, pending;
    try { contentKey = contentKey || StickyCrypto.generateContentKey(); payload = payload || StickyCrypto.encryptJson(note, contentKey); } catch (error) { Fsn.toast('付箋の暗号化に失敗しました。'); return; }
